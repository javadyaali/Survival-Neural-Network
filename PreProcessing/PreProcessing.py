...
def preProcessing():
    inputFile = pd.read_csv('/home/shayan/Desktop/javad/lastfm-dataset-1K/userid-timestamp-artid-artname-traid-traname.tsv',
                    sep='\t',
    ...
    allActionList = list(set(inputFile.loc[:, "artid"]))
        ...
        if iteration != 1:
            ...
            if (newUserNumber - oldUserNumber) == 0:
                ...
                if (holeSize <= (180*24)) and (holeSize >= gapThreshold):
                    ...
                    sessionActionsList = list(inputFile.loc[beginSessionIndex:endSessionIndex, "artid"])
                    sessionActionsDict = allActionDict
                    for item in sessionActionsList:
                        if isinstance(item,str):
                            sessionActionsDict[item] += 1
                    ...
                    output.append([newUserNumber, gapSize, returnDayName, returnDayHour, sessionActionsDict, beginSessionTimestamp, endSessionTimestamp])
